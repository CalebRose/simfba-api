...
const getFileAsString = (filePathStr): string => {
...
const getFilePaths = (dir: string): IPathNameObj[] => {
    const filePaths = [];
...
const createModelIfNeeded = (pathNameObj: IPathNameObj, keyNames: string[], tableDataObj: {}): boolean | undefined => {
    const createModel = (): void => {
        console.log("\n\t\tCreating Model:\t" + pathNameObj.filename);
        console.log("\nColumn names:\t" + keyNames + "\n");
        const inferColumnType = (i: number): string => {
            // eslint-disable-next-line @typescript-eslint/tslint/config
        ...
        const finalModelName = removedExtFilename.endsWith("s") ? removedExtFilename.slice(0, removedExtFilename.length - 1) : removedExtFilename ;
...
const checkDirectorySync = (directory): void => {
    try {
